...
parser = PDPC.add_model_specific_args(parser)

args = parser.parse_args()


def create_logger_name_for_args(args):
    name = ["PDPC", args.dataset]
    ...
    name.append("sigma_decay_{}".format(args.sigma_decay))
    name.append("pi_entropy_{}".format(args.pi_entropy))
    name.append("n_conv_{}".format(args.n_conv))
    name.append("combine_at_{}".format(args.combine_at))
    name.append("active_scale_inds_{}".format(args.active_scale_inds).replace(" ", ""))
    name.append("inner_channel{}".format(args.inner_channel))
    return "_".join(name)
...
def main(args):
    ...
    if args.test:
        model = PDPC.load_from_checkpoint(args.checkpoint)
        trainer.test(model)
    else:
        model = PDPC(args)
        # trainer.tune(model)
        trainer.fit(model)
        model = PDPC.load_from_checkpoint(os.path.join(args.save_dir, name + ".ckpt"))
