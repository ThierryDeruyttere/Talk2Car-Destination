...
def main(args):
    ...
    for bidx, data in enumerate(tqdm(test_loader)):
        ...
        object_boxes, referred_object, ego_car = data["object_boxes"], data["referred_object"], data["ego_car"]
        inp = inp.float().to(device)
        gt_coord = gt_coord.float().to(device)
        command = command.to(device)
        object_boxes = object_boxes.mean(dim=2).to(device)
        referred_object = referred_object.unsqueeze(1).mean(dim=2).to(device)
        ego_car = ego_car.unsqueeze(1).mean(dim=2).to(device)

        """PRED COORD"""
        pred_object_indices = random.choices(
            list(range(ego_car.shape[1])), k=1000
        )
        pred_coord = ego_car[:, pred_object_indices]
        """PRED COORD"""
