...
def main(args):
    ...
    for bidx, data in enumerate(tqdm(test_loader)):
        ...
        """PRED COORD"""
        road_plan = inp[:, :3]
        road_plan = road_plan.max(dim=1)[0]
        candidate_indices = (road_plan < 1).nonzero()
        pred_coord = candidate_indices[torch.randperm(candidate_indices.shape[0])[:1000]][:, 1:]
        if pred_coord.shape[0] == 1000:
            pred_coord = pred_coord.flip(dims=(1,)).unsqueeze(0).to(gt_coord)
            pred_coord = pred_coord / torch.tensor([road_plan.shape[2], road_plan.shape[1]]).to(gt_coord)
            # SAMPLE PIXEL WITH COLOR RGB OF THE ROAD
            """PRED COORD"""
