...
class Talk2Car_Detector(Dataset):
    ...
    def __getitem__(self, ix):
        ...
        object_boxes = torch.tensor(detection_boxes) / torch.tensor([self.orig_width, self.orig_height])
        referred_object = torch.tensor(detection_boxes[detection_pred_box_index]) / torch.tensor([self.orig_width, self.orig_height])
        ego_car = torch.tensor(ego_car_top) / torch.tensor([self.orig_width, self.orig_height])
