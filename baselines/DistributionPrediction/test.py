...
import mvg_independent, mvg_dependent
...
def get_grid_logprob(height, width, distr, device):
...
def main(args):
    ...
    model = DistributionPrediction(hparams)
    ...
    if hparams["mvg_type"] == "dependent":
        criterion = mvg_dependent.mvg_loss
    elif hparams["mvg_type"] == "independent":
        criterion = mvg_independent.mvg_loss
    ...
    for bidx, data in enumerate(tqdm(test_loader)):
        ...
        mu, tril_or_sigma = model.forward(x, command)
        ...
        if args.draw and counter < args.num_heatmaps_drawn:
            ...
            log_prob_grid, X, Y = get_grid_logprob(height, width, distr, device)
