...
class AnchorFreeMono3DHead(BaseMono3DDenseHead):
    ...
    def forward_single(self, x):
        ...
        for cls_layer in self.cls_convs:
            cls_feat = cls_layer(cls_feat) # Still C x H x W after this
        ...
        for conv_cls_prev_layer in self.conv_cls_prev:
            clone_cls_feat = conv_cls_prev_layer(clone_cls_feat)  # Still C x H x W after this
        cls_score = self.conv_cls(clone_cls_feat) # clone_cls_feat are the features of the objects for classification
        final_obj_repr.append(clone_cls_feat)
        for reg_layer in self.reg_convs:
            reg_feat = reg_layer(reg_feat)  # Still C x H x W after this
        bbox_pred = []

        # self.group_reg_dims = (2, 1, 3, 1, 2), see comment below
        for i in range(len(self.group_reg_dims)):
            # Here we are going to predict offset, depth, size, rot, velo by using the following channels respectively
            # (2, 1, 3, 1, 2). i.e. for the offset we go from C (=256) to 2 channels.
            # For depth we go from C to 1 channel
            ...
            final_obj_repr.append(clone_reg_feat)
        ...
        return cls_score, bbox_pred, dir_cls_pred, attr_pred, cls_feat, \
            reg_feat, torch.cat(final_obj_repr, dim=1)
    ...
    def get_points(self, featmap_sizes, dtype, device, flatten=False):
        ...
        return mlvl_points # For each position in the feature map it shows the corresponding position in the original image
