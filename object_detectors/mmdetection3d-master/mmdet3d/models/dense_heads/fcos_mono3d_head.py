...
class FCOSMono3DHead(AnchorFreeMono3DHead):
    ...
    def forward_single(self, x, scale, stride):
        ...
        """
        cls_score, bbox_pred, dir_cls_pred, attr_pred, cls_feat, reg_feat, final_obj_repr = \
        ...
        # only apply to offset, depth and size prediction. These scales are a module from mmcv where they learn one parameter: scale. The scale module just multiplies this scale with the values given to it. With this they can do predictions at different levels of scale!
        ...
        return cls_score, bbox_pred, dir_cls_pred, attr_pred, centerness, final_obj_repr
    ...
    def get_bboxes(self,
        ...
        featmap_sizes = [featmap.size()[-2:] for featmap in cls_scores] # H,W for each scale in the FPN
        ...
        for img_id in range(len(img_metas)):
            ...
            final_obj_repr_list = [
                final_obj_repr[i][img_id].detach() for i in range(num_levels)
            ]
            ...
            det_bboxes = self._get_bboxes_single(
                ...
                cfg, final_obj_repr_list, rescale)
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, dir_cls_pred, attr_pred, centerness, \
                points, feat_obj_repr in zip(cls_scores, bbox_preds, dir_cls_preds,
                              attr_preds, centernesses, mlvl_points, final_obj_repr):
            assert cls_score.size()[-2:] == bbox_pred.size()[-2:]
            scores = cls_score.permute(1, 2, 0).reshape(
                -1, self.cls_out_channels).sigmoid()
            scores_softmax = cls_score.permute(1, 2, 0).reshape(
                -1, self.cls_out_channels).softmax(-1)
            ...
            bbox_pred = bbox_pred.permute(1, 2,
                                          0).reshape(-1,
                                                     sum(self.group_reg_dims)) # sum(group_reg_dims) = sum(2, 1, 3, 1, 2) = 9
            bbox_pred = bbox_pred[:, :self.bbox_code_size]
            obj_repr = feat_obj_repr.permute(1,2,0).reshape(scores.shape[0], -1)
            ...
            if nms_pre > 0 and scores.shape[0] > nms_pre:
                ...
                scores_softmax = scores_softmax[topk_inds, :]
                dir_cls_pred = dir_cls_pred[topk_inds, :]
                centerness = centerness[topk_inds]
                dir_cls_score = dir_cls_score[topk_inds]
                attr_score = attr_score[topk_inds]
                obj_repr = obj_repr[topk_inds]

            # change the offset to actual center predictions
            bbox_pred[:, :2] = points - bbox_pred[:, :2] # get real image centers of object
            if rescale:
                bbox_pred[:, :2] /= bbox_pred[:, :2].new_tensor(scale_factor)
            pred_center2d = bbox_pred[:, :3].clone() # centers of boxes of format (x,y,depth)
            ...
            mlvl_obj_repr.append(obj_repr)
            mlvl_scores_softmax.append(scores_softmax)

        mlvl_centers2d = torch.cat(mlvl_centers2d)
        mlvl_bboxes = torch.cat(mlvl_bboxes)
        mlvl_dir_scores = torch.cat(mlvl_dir_scores)
        mlvl_obj_repr = torch.cat(mlvl_obj_repr)
        ...
        mlvl_scores_softmax = torch.cat(mlvl_scores_softmax)
        ...
        results = box3d_multiclass_nms(mlvl_bboxes, mlvl_bboxes_for_nms,
                                       ...
                                       mlvl_attr_scores, obj_repr=mlvl_obj_repr,
                                       cls_scores=mlvl_scores_softmax)
        bboxes, scores, labels, dir_scores, attrs, feats, cls_scores = results
        ...
        return bboxes, scores, labels, attrs, feats, cls_scores
