...
class SingleStageMono3DDetector(SingleStageDetector):
    ...
    def simple_test(self, img, img_metas, rescale=False):
        ...
        """
        x = self.extract_feat(img)
        outs = self.bbox_head(x) # outs is a 5 tuple (cls_score, bbox_pred, dir_cls_pred, attr_pred, centerness) that contains a list that contain 5 elements (for 5 scales)
        outs, final_obj_repr = outs[:5], outs[5]
        bbox_outputs = self.bbox_head.get_bboxes(
            *outs, img_metas, final_obj_repr, rescale=rescale)
        ...
        bbox_img = [
            ...
            for bboxes, scores, labels, attrs, feats, cls_score in bbox_outputs
        ]

        bbox_list = [dict() for i in range(len(img_metas))]
        for result_dict, img_bbox, outputs in zip(bbox_list, bbox_img, bbox_outputs):
            #print(feats)
            result_dict['img_bbox'] = img_bbox
            result_dict['img_bbox']["feats"] = outputs[-2].cpu()
            result_dict['img_bbox']["cls_scores"] = outputs[-1].cpu()
