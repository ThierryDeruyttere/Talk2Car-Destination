...
def box3d_multiclass_nms(mlvl_bboxes,
    ...
    for i in range(0, num_classes):
        ...
        if "obj_repr" in kwargs:
            feats.append(kwargs["obj_repr"][selected])
        if "cls_scores" in kwargs:
            cls_scores.append(kwargs["cls_scores"][selected][:,:10])
    ...
    if bboxes:
        ...
        if "obj_repr" in kwargs:
            feats = mlvl_scores.new_zeros((0, kwargs["obj_repr"].size(-1)))
        if "cls_scores" in kwargs:
            cls_scores = mlvl_scores.new_zeros((0, num_classes))
        if mlvl_dir_scores is not None:
    ...
    if "obj_repr" in kwargs:
        results = results + (feats, )
    if "cls_scores" in kwargs:
        results = results + (cls_scores, )
