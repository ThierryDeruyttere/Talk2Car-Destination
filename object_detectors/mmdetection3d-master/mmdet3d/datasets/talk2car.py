...
class Command:
    ...
    def get_2d_bbox(self):
        ...
        :return: A tuple of a bounding box as (x1, y1, rzidth, height)
        """
        if self.t2c.version == "t2c_challenge_test_set":
            return self.box
    ...
    def get_image_path_and_cam_intr(self):
        """
        :return:
        """
        # Get records from DB
        ...
        # Get data from DB
        im_path, _, camera_intrinsic= self.t2c.get_sample_data(sd_rec["token"])
        return im_path, camera_intrinsic
...
class Talk2Car(NuScenes):
    ...
    def __init__(
        ...
        """
        if version == "t2c_challenge_test_set":
            super().__init__(version='v1.0-test', dataroot=dataroot, verbose=verbose)
        else:
    ...
    def __load_t2c_table__(self, table_name):
        with open(
                osp.join(
                    ...
                    "new/{}_commands.json".format(table_name),
                )
        ) as f:
            table = json.load(f)
        return table
    ...
    def __load_commands__(self) -> [List, Dict]:
        ...
        if self.version == "t2c_challenge_test_set":
            for c in cmnds:
                scene_token = c["scene_token"]
                box = c["gt_bbox"]
                command_text = c["command"]

                cmd = Command(self, scene_token, c["sample_token"], box, command_text, None,
