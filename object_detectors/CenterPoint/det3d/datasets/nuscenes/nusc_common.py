...
import sys
from det3d.datasets.nuscenes.talk2car import Talk2Car
...
def create_t2c_infos(root_path, version="train", nsweeps=10, filter_zero=True):
    nusc = Talk2Car(version=version, dataroot=root_path, verbose=True)
    #print("finished")
    if version == "train":
        train_scenes = set([c.scene_token for c in nusc.commands])
        # random.shuffle(train_scenes)
        # train_scenes = train_scenes[:int(len(train_scenes)*0.2)]
        nusc.change_version("val")
        val_scenes = set([c.scene_token for c in nusc.commands])
    elif version == "test":
        train_scenes = set([c.scene_token for c in nusc.commands])
        val_scenes = []
    elif version == "v1.0-test":
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, train_scenes, val_scenes, False, nsweeps=nsweeps, filter_zero=filter_zero
    )
...
if __name__ == "__main__":
    sys.path.append("/cw/liir/NoCsBack/testliir/thierry/PathProjection/3d_object_detection/CenterPoint/nuscenes-devkit/python-sdk")
    create_t2c_infos("/export/home2/NoCsBack/hci/thierry/datasets/nuScenes/data/sets/nuscenes", version="train", nsweeps=10, filter_zero=True)