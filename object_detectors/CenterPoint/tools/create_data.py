...
def t2c_data_prep(root_path, version, nsweeps=10, filter_zero=True):
    nu_ds.create_t2c_infos(root_path, version=version, nsweeps=nsweeps, filter_zero=filter_zero)
    create_groundtruth_database(
        "NUSC",
        root_path,
        Path(root_path) / "infos_train_{:02d}sweeps_withvelo_filter_{}.pkl".format(nsweeps, filter_zero),
